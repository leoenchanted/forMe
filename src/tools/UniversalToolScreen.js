...
import { View, StyleSheet, ActivityIndicator, Alert, Share, Modal, TouchableOpacity, Text, ScrollView, Platform } from 'react-native';
...
export default function UniversalToolScreen({ route, navigation }) {
    ...
    // 复制功能，这里不需要，因为 HTML 已经自己处理复制逻辑并发送 haptic 消息
    ...
    const saveWallpaper = useCallback(async () => {
        ...
        const filename = FileSystem.cacheDirectory + `mesh_gradient_${Date.now()}.png`;
        ...
        >
            <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                    ...
                    <ScrollView style={styles.dataScrollView}>
                        {exportColors.map((color, index) => (
                            <View key={index} style={styles.colorRow}>
                                <View style={[styles.colorChip, { backgroundColor: color.hex }]} />
                                ...
                                <View style={styles.colorInfo}>
                                    ...
                                    <Text style={styles.colorValue}>R:{color.r} G:{color.g} B:{color.b}</Text>
                                </View>
                            </View>
                        ))}
                    </ScrollView>
                    ...
                    <TouchableOpacity style={styles.closeActionBtn} onPress={() => setIsExportModalVisible(false)}>
...
const styles = StyleSheet.create({
    ...
    modalContent: { width: '85%', maxWidth: 400, backgroundColor: '#2c2c2e', borderRadius: 20, padding: 20, alignItems: 'center' },
    modalTitle: { color: 'white', fontSize: 20, fontWeight: '700', marginBottom: 15 },
    
    dataScrollView: { maxHeight: 200, width: '100%', marginBottom: 15 },
    colorRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1c1c1e', borderRadius: 10, padding: 10, marginBottom: 8 },
    colorChip: { width: 30, height: 30, borderRadius: 6, marginRight: 15, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
    colorInfo: { flex: 1, marginRight: 10 },
    colorLabel: { color: '#8e8e93', fontSize: 10, marginBottom: 2 },
    colorValue: { color: 'white', fontSize: 12, fontWeight: '500', fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace' },

    tipBox: { backgroundColor: 'rgba(245, 158, 11, 0.1)', padding: 10, borderRadius: 10, width: '100%', marginBottom: 20, flexDirection: 'row', justifyContent: 'center' },
    tipText: { color: '#f59e0b', fontSize: 13, textAlign: 'center' },
    
    saveActionBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#007aff', padding: 15, borderRadius: 14, width: '100%', marginBottom: 10 },
    saveActionText: { color: 'white', fontSize: 16, fontWeight: '600', marginLeft: 8 },
